...
import { Scissors, ClipboardList, Loader2, Pencil, Trash2 } from "lucide-react";
...
export default function PaginaCorte() {
            ...
            try {
                ...
                snap.forEach(doc => {
                    ...
                    fetchedRecords.push({
                        ...
                        total: data.total_cortado || 0,
                        fecha: data.fecha_registro?.toDate ? data.fecha_registro.toDate() : new Date(),
                    });
                });
                setRecords(fetchedRecords);
            } catch (error) {
                console.error("Error fetching historical cutting records:", error);
            }
        };
        fetchHistory();
    }, []);
    ...
    const handleAgregarRegistro = async () => {
        ...
        try {
            ...
            setVariantes([]);
            setTipoProducto('');
            toast.success(`Lote de corte #${idRegistro} guardado con ${totalLote} prendas en total.`);
        } catch (error) {
            console.error(error);
    ...
    const totalCortadoHoy = records.reduce((acc, current) => acc + current.total, 0);

    return (
        <ProtectedRoute allowedRoles={["master", "cutting"]}>
            <div className="space-y-6">
                <h1 className="text-3xl font-bold tracking-tight text-stone-900 dark:text-white">Área de Corte</h1>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="bg-white dark:bg-zinc-950 p-6 rounded-2xl shadow-sm border border-stone-200 dark:border-zinc-800 flex items-center">
                        <div className="p-4 bg-purple-100 dark:bg-purple-500/20 text-purple-600 rounded-xl mr-4">
                            <Scissors className="w-8 h-8" />
                        </div>
                        <div>
                            <p className="text-sm text-stone-500 font-medium">Histórico: Prendas Cortadas</p>
                            <p className="text-3xl font-bold text-stone-900 dark:text-white">{totalCortadoHoy}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid lg:grid-cols-3 gap-6">
                    ...
                    <div className="lg:col-span-1 bg-white dark:bg-zinc-950 p-6 rounded-2xl shadow-sm border border-stone-200 dark:border-zinc-800 h-fit">
                        <h2 className="text-xl font-bold mb-6 flex items-center">
                            <ClipboardList className="w-5 h-5 mr-2 text-purple-600" /> Registrar Corte
                        </h2>
                        <form onSubmit={handleAgregarRegistro} className="space-y-5">
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <select
                                            ...
                                            className="flex-1 px-4 py-2 rounded-lg border border-stone-200 dark:border-zinc-800 bg-stone-50 dark:bg-zinc-900 outline-none focus:ring-2 focus:ring-purple-500"
                                            required
                                        >
                                            ...
                                            <option value="___NUEVO___" className="text-purple-600 font-bold">+ Añadir Nueva...</option>
                                        </select>
                                        {categoria && (
                                ...
                                <div>
                                    ...
                                    <div className="flex gap-2">
                                        <select
                                            ...
                                            className="flex-1 px-4 py-2 rounded-lg border border-stone-200 dark:border-zinc-800 bg-stone-50 dark:bg-zinc-900 outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50"
                                            required
                                        >
                                            <option value="" disabled>Seleccione...</option>
                                            ...
                                            <option value="___NUEVO___" className="text-purple-600 font-bold">+ Añadir Nuevo...</option>
                                        </select>
                                        {tipoProducto && (
                                            <div className="flex gap-1">
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full px-4 py-2 rounded-lg border border-stone-200 dark:border-zinc-800 bg-stone-50 dark:bg-zinc-900 outline-none focus:ring-2 focus:ring-purple-500"
                                        required
                                    >
                                        <option value="" disabled>Seleccione...</option>
                                ...
                                <div>
                                    ...
                                    <select
                                        ...
                                        className="w-full px-4 py-2 rounded-lg border border-stone-200 dark:border-zinc-800 bg-stone-50 dark:bg-zinc-900 outline-none focus:ring-2 focus:ring-purple-500 disabled:opacity-50"
                                        required
                                    >
                                        <option value="" disabled>Seleccione...</option>
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                ...
                                <div>
                                    <label className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-1">Cant. Cortada</label>
                                    <input
                                        type="number" min="1"
                                        value={cantidad} onChange={e => setCantidad(Number(e.target.value))}
                                        className="w-full px-4 py-2 rounded-lg border border-stone-200 dark:border-zinc-800 bg-stone-50 dark:bg-zinc-900 outline-none focus:ring-2 focus:ring-purple-500 font-bold"
                                    />
                                </div>
                            </div>

                            <button type="button" onClick={handleAgregarVariante} className={`w-full py-4 flex items-center justify-center font-black rounded-xl transition-all shadow-md text-white bg-purple-600 hover:bg-purple-700 hover:shadow-lg active:scale-[0.98]`}>
                            ...
                            <button type="button" onClick={handleAgregarRegistro} disabled={isSubmitting || variantes.length === 0} className="w-full py-4 mt-2 bg-stone-900 dark:bg-white text-white dark:text-stone-900 font-bold rounded-lg hover:bg-stone-800 dark:hover:bg-stone-200 transition-colors shadow-sm disabled:opacity-50 flex justify-center items-center">
                                {isSubmitting ? (
                                    <><Loader2 className="w-5 h-5 mr-2 animate-spin" /> Guardando Lote...</>
                                ) : (
                                    "Guardar Lote de Corte"
                                )}
                            </button>
                        </form>
                    </div>
                    ...
                    <div className="lg:col-span-2 bg-white dark:bg-zinc-950 p-6 rounded-2xl shadow-sm border border-stone-200 dark:border-zinc-800 overflow-hidden flex flex-col h-full">
                        <h2 className="text-xl font-bold mb-4">Historial de Lotes de Corte</h2>
                                    ...
                                    {records.length === 0 ? (
                                        <tr>
                                            <td colSpan={5} className="px-4 py-12 text-center text-stone-500 border-2 border-dashed border-stone-200 dark:border-zinc-800 rounded-xl mt-4">
                                                No hay lotes de corte registrados en el historial general.
                                            </td>
                                        </tr>
                                    ) : (
